...
{
    auto & sample = getOutputs()[0].getHeader();
...
void JSONRowOutputFormat::writeTotalsField(const IColumn & column, const IDataType & type, size_t row_num)
{
    ...
    writeCString(": ", *ostr);
    type.serializeAsTextJSON(column, 0, *ostr, settings);
    ++field_number;
}
...
void registerOutputFormatProcessorJSON(FormatFactory & factory)
{
        ...
        const FormatSettings & format_settings)
    {
        return std::make_shared<BlockOutputStreamFromRowOutputStream>(
            std::make_shared<JSONRowOutputFormat>(buf, sample, format_settings), sample);
    });
}
